...
- [ ] I-12 Secrets manager integration for staging/prod (Vault or AWS Secrets Manager) + rotation automation. <!-- TDD Sections 10.6 & 13.7 -->
- [ ] I-13 Container image signing & verification with `cosign` and pinned digest enforcement in CD. <!-- TDD Sections 10.5 & 9.3 -->
...
- [ ] B-CC-9 Read-only mode kill-switch + guarded mutating endpoints (toggle via config/feature flags). <!-- TDD Section 13.8 -->
...
- [x] B-USR-1 User CRUD + status transitions (pending → active → archived) with audit entries (TDD §4.2.1)
- [x] B-USR-2 Contact management (`email`, `phone`) and verification flags (TDD §6.2.5)
- [x] B-USR-3 Avatar upload + media linkage through storage abstraction (TDD §4.2.3, §6.5.2)
- [x] B-USR-4 GDPR delete/export endpoints tied to retention routines (PRD §5.2, TDD §12.6)
...
- [x] B-EX-1 Admin seed + user-owned exercises with ownership rules and soft delete (PRD §4.3, TDD §4.3.2)
- [x] B-EX-2 Tagging/search endpoints backed by JSONB GIN index (TDD §6.2.7)
- [x] B-EX-3 Exercise type lookup APIs sourced from normalized tables (TDD §6.2.6)
- [x] B-EX-4 Global exercise admin seed pack (Squat/Bench/Deadlift/Run/Ride with categories & tags).

### 4.4 Planning & Sessions (FR-4)

- [x] B-SES-1 Session CRUD (`plan_id`, status transitions) with recurrence metadata (TDD §4.4.2, §6.2.9)
- [x] B-SES-2 Session exercise + attribute management (planned vs actual) including validation (TDD 4.4.3, 6.2.10-6.2.11)
- [x] B-SES-3 Clone/template + idempotent create behavior (TDD §4.4.4)
- [x] B-SES-4 Session summary refresh hooks / materialized view maintenance (TDD §6.7, §12.4)
- [x] B-SES-5 Recurrence templates API (clone with date offset & safeguards).

### 4.5 Progress & Analytics (FR-5)

- [x] B-PRG-1 Summary & trends endpoints using exercise set metrics (`/progress/summary`, `/progress/trends`) (TDD §4.5.1–4.5.2)
- [x] B-PRG-2 Exercise breakdown & plans progress endpoints with audit logging (TDD §4.5.3)
- [x] B-PRG-3 Export/report scaffolding aligned with QA analytics checks (PRD §5.3.5, QA §8.3)
...
- [ ] F-10 Visibility badges & guards (clear “Private/Link/Public” indicators wherever content appears).
...
- [ ] F-15 Runtime feature flag & kill-switch sync (fetch `/config`, hide analytics, surface read-only banner). <!-- TDD Sections 13.5 & 13.8 -->
