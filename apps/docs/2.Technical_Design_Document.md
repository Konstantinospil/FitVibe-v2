...
> Any gate failure **blocks** the pipeline (see Â§9.9).
...
2. **Switch** to previous image digests (kept per Â§10.3.3).
...
- **sessions** (monthly partitions):
  - **Create** next monthâ€™s partition 7 days before month-end.
...
- **Objective**: RTO â‰¤ 4 h, RPO â‰¤ 24 h.
...
- JWT Key Rotation â†’ Â§12.4
- Session Revocation â†’ Â§12.5
- DSR Export/Delete â†’ Â§12.6
- Release Rollback â†’ Â§12.7
- Backup & Restore Drill â†’ Â§12.8
- Partition Rotation â†’ Â§12.9
- Media AV Failure â†’ Â§12.10
- Disaster Recovery â†’ Â§12.11
...
**Outcome**: These runbooks make operations repeatable, auditable, and aligned with FitVibeâ€™s security, privacy, and reliability targets. Keep them **current** with every release and reflect exceptions in ADRs.

---

# 13. Environments & Configuration Matrix

This section specifies **environment topology**, **configuration schema**, **required/optional variables**, **defaults**, **rotation policies**, **feature flags**, and **change control** for **Local**, **Staging**, and **Production**. It binds to Â§5.6 (Configuration & Secrets), Â§9 (NFRs), and Â§10 (CI/CD).
...
- **Metrics** (`/metrics`): exposes standard metrics (see Â§5.4).
...
- **JWT keys** rotated per Â§12.4; other secrets rotated **quarterly** or on incident.
...
| ---- | ---------------------------------------------------------------------- | --- | --- | ----- | ---------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ | ----- | ------ |
| R-01 | **Bus factor = 1** (solo owner); delayed incident response or releases | H   | H   | **H** | Documented runbooks (Â§12); CI/CD automation (Â§10); infra as code; keep rollback one command away; small, frequent releases | If on-call unavailable â†’ extend maintenance window; pause feature work; prioritize automation & docs | Owner | Open   |
| R-02 | API performance misses p95 < 300 ms under spikes                       | M   | H   | **H** | Hot-index map (Â§6.3); k6 gates (Â§9.2.4); caching/ETag; DB timeouts                                                         | Auto-scale API; enable read-only mode for heavy analytics                                              | Owner | Open   |
| R-03 | Security headers or CORS misconfig in prod                             | M   | H   | **H** | NGINX template checked-in; CI header tests; ZAP baseline (Â§9.3)                                                             | Immediate config rollback; block deploys until fixed                                                   | Owner | Open   |
| R-04 | Token reuse/refresh rotation bugs â†’ session hijack window            | L   | H   | **M** | Reuse detection, family revoke; short access TTL; deterministic tests (Â§11)                                                 | Force global logout; rotate keys (Â§12.4)                                                              | Owner | Open   |
| R-05 | GDPR DSR delete not fully propagated to backups in â‰¤14d              | M   | M   | **M** | Delete queue with audit trail; backup purge checklist (Â§12.6)                                                               | Temporarily disable new signups; manual purge + postmortem                                             | Owner | Open   |
| R-06 | Media uploads accept malware or PII leaks via EXIF                     | M   | M   | **M** | AV scan + EXIF strip; MIME allow-list; quarantine and alerts (Â§5.1.6, Â§12.10)                                              | Block uploads; rotate allowâ€‘list; incident notice if required                                        | Owner | Open   |
| R-07 | Analytics queries exceed 1 MB / time budgets                           | M   | M   | **M** | Enforce server-side slicing; matviews; CI perf gates                                                                         | Temporarily disable analytics endpoints (Â§9.5)                                                        | Owner | Open   |
...
| R-12 | Schema migrations cause locks/outage                                   | L   | H   | **M** | Expand/contract, statement timeouts, staging dry-run (Â§10.4)                                                                | Rollback app; forward-fix migration                                                                    | Owner | Open   |
| R-13 | Observability cardinality explosion                                    | M   | M   | **M** | Label allow-list; route templates; alerts on series count (Â§5.4)                                                            | Drop high-card labels; reduce sample rate                                                              | Owner | Open   |
| R-14 | Legal/ToS exposure from public feed content                            | L   | H   | **M** | Private by default; takedown workflow; audit for admin actions (Â§4.7, Â§5.2)                                                | Disable public feed flag; remove offending content                                                     | Owner | Open   |
...
1. **MVP user base** is modest (â‰¤ a few thousand monthly active), allowing single-node API + managed Postgres to meet SLOs.
...
6. **Single region** hosting is acceptable for MVP with DR plan in Â§12.11.
...
- **GDPR**: DSR export & delete supported; backup purge â‰¤ **14 days** after deletion completion.
- **Performance**: API **p95 < 300 ms**; FE **LCP < 2.5 s** (P75); payload caps (req â‰¤ 2 MB, resp â‰¤ 1 MB).
- **Quality**: **Coverage â‰¥ 80%** repo-wide; ZAP baseline no high findings; k6/Lighthouse budgets enforced in CI.
- **Reliability**: RPO â‰¤ 24 h; RTO â‰¤ 4 h; health/readiness endpoints mandatory.
...
This section catalogs the **Architecture Decision Records (ADRs)** that govern FitVibeâ€™s design and implementation, and provides a consolidated **reference list** to source documents. Every material change to architecture, data, security, or contracts must be captured by an ADR and cross-linked from the PRD, TDD, and QA Plan (traceability).
...
- **Status lifecycle**: `Proposed â†’ Accepted â†’ Superseded â†’ Deprecated`.
- **Change control**: Any PR touching Â§5 (security), Â§6 (data), Â§7 (API), or Â§9 (NFRs) must reference an **ADR** (new or existing).
- **Linking**: Each ADR must link the affected sections: PRD Â§_, TDD Â§_, QA Â§\*, and list impacted tests/gates.
...
- Alternative A â€” pros/cons
- Alternative B â€” pros/cons
...
1. **Create file** `docs/adr/ADR-XYZ-title.md` from the template in Â§15.1.1.
2. Fill **Context â†’ Decision â†’ Consequences â†’ Alternatives**.
3. Cross-link affected sections in PRD/TDD/QA.
4. Add to table in Â§15.2 with `Proposed` status.
5. Submit PR; on approval, set status to `Accepted` and add decision date.
6. If superseding a prior ADR, update both recordsâ€™ headers accordingly.
...
Stable `error.code` â†’ HTTP mapping with user-facing guidance. Messages are localized on the client.
...
| `E.AUTH.TOTP_REQUIRED`           | 401     | Auth      | Twoâ€‘factor code required  | Provide OTP or disable 2FA with backup  |
...
| `E.SESSION.INVALID_SET`          | 422     | Sessions  | Invalid set values          | Fix nonâ€‘negative constraints          |
...
Quick reference to major endpoints (full contracts in **TDD Â§7**).
...
Field/table-level classification, lawful basis, retention, and handling. Classes: **PIIâ€‘S** (Sensitive), **PIIâ€‘B** (Basic), **Usage**, **Public**.

| Table.Field                       | Class         | Lawful Basis                   | Retention (Default)      | Access Scope               | Handling Notes              |
| --------------------------------- | ------------- | ------------------------------ | ------------------------ | -------------------------- | --------------------------- |
| users.email                       | PIIâ€‘S       | Contract                       | Lifetime (acct)          | User/Admin                 | citext unique; never logged |
| users.alias                       | PIIâ€‘B       | Contract                       | Lifetime                 | Public (if profile public) | case-insensitive unique     |
| users.password_hash               | PIIâ€‘S       | Contract                       | Lifetime                 | None                       | bcryptjs; never exported    |
| users.locale                      | PIIâ€‘B       | Legitimate interest            | Lifetime                 | User/Admin                 | defaults to 'en'            |
| profiles.avatar_asset_id          | PIIâ€‘B       | Consent                        | Lifetime                 | User/Admin                 | image; AV scan              |
| user_sessions.ip                  | PIIâ€‘S       | Legitimate interest (security) | 90d active + 30d revoked | Admin                      | inet; masked in UI          |
| verification_tokens.token         | PIIâ€‘S       | Contract                       | TTL (15â€“60m)           | System                     | one-time; auto-purge        |
| exercises.\* (user-owned)         | Usage/PIIâ€‘B | Contract                       | Lifetime of user         | User                       | hard-delete on DSR          |
...
| media_assets.mime/size/av_scan    | PIIâ€‘B       | Legitimate interest            | Lifetime                 | User/Admin                 | allow-list; quarantine      |
| audit_log.user_id                 | PIIâ€‘S       | Legitimate interest            | 180 days                 | Admin                      | anonymize on DSR            |
