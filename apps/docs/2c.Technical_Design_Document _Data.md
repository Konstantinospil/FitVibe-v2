...
- **feed_items** � activity feed entries (sessions, badges, milestones) with visibility stored as constrained text values (public, ollowers, link, private).
- **feed_comments**, **feed_likes**, **session_bookmarks**, **followers**, **share_links** � social interactions. Share-links enforce a single target (session XOR feed item) via constraint.
- **badge_catalog**, **badges** � badge definitions and awarded instances (metadata captures unlock context).
- **user_points** � append-only ledger of points with partitioning on warded_at.
...
| category    | text        | taxonomy (e.g., `milestone`, `streak`)  |
| icon        | text        | optional asset key                      |
| priority    | integer     | default `0`; determines surfacing order |
| criteria    | jsonb       | machine-readable unlock rules           |
| created_at  | timestamptz | default `now()`                         |

Reference catalog seeded via migration; admins manage additional badges offline.
...
| id         | uuid        | **pk**                                |
| user_id    | uuid        | **fk ? users.id (ON DELETE CASCADE)** |
| badge_type | text        | **fk ? badge_catalog.code**           |
| metadata   | jsonb       | unlock context snapshot               |
| awarded_at | timestamptz | default `now()`                       |

**Unique:** `(user_id, badge_type)` prevents duplicates.
...
| id                | uuid        | **pk**                                             |
| user_id           | uuid        | **fk ? users.id (ON DELETE CASCADE)**              |
| source_type       | text        | `session.completed`, `streak`, `referral`, etc.    |
| source_id         | uuid        | optional dedupe key                                |
| algorithm_version | text        | scoring model identifier                           |
| points            | integer     | not null                                           |
| calories          | integer     | optional payload                                   |
| metadata          | jsonb       | extensible context blob                            |
| awarded_at        | timestamptz | default `now()`; **partition key** (monthly range) |
| created_at        | timestamptz | default `now()`                                    |

**Index:** `(user_id, awarded_at DESC)`. Partial unique `(user_id, source_type, source_id)` when `source_id` is present.
...
| kind         | text          | `session`, `badge`, `milestone`, etc. (constrained text) |
| target_type  | text          | optional discriminator                                   |
| target_id    | uuid          | optional target reference                                |
| visibility   | text          | `public`, `followers`, `link`, `private`                 |
| score        | numeric(10,2) | ranking weight                                           |
| published_at | timestamptz   | nullable                                                 |
| created_at   | timestamptz   | default `now()`                                          |
| updated_at   | timestamptz   | default `now()`                                          |
| deleted_at   | timestamptz   | nullable                                                 |

Partial index on `(visibility, published_at DESC)` for public feed; `(owner_id, published_at DESC)` for personal view.
...
Composite **pk** `(feed_item_id, user_id)` with covering index `(feed_item_id, created_at DESC)`.
...
**Check:** exactly one of `session_id` or `feed_item_id` is populated. Index `(session_id)` supports lookup.
...
Composite **pk** `(follower_id, following_id)` with check `follower_id <> following_id`.
...
Composite **pk**, check `blocker_id <> blocked_id`, plus indexes on `(blocker_id)` and `(blocked_id)` for enforcement queries.
...
| feed_item_id | uuid        | **fk ? feed_items.id (ON DELETE SET NULL)**       |
| comment_id   | uuid        | **fk ? feed_comments.id (ON DELETE SET NULL)**    |
| reason       | text        | normalized reason code                            |
...
Indexes on `(feed_item_id, status)` and `(comment_id, status)` support queue views.

---

#### 6.3.6.12 mv_leaderboard (materialized view)

Columns: `period_type` (`week`/`month`), `period_start` (date), `user_id`, `username`, `display_name`, `points`, `badges_count`.
The view aggregates `user_points` per period and is refreshed by `refresh-materialized.ts`. Unique index `(period_type, period_start, user_id)` plus sort index `(period_type, period_start DESC, points DESC)`.
...
| badges (catalog)               | Public/Admin                   | N/A                     | Keep                                                                               |
