...
export async function createType(req: Request, res: Response) {
  ...
  const adminId = (req as any).user?.sub;  // get authenticated admin ID
  const type = await addType(parsed.data, adminId); // pass to service
...
export async function updateType(req: Request, res: Response) {
  ...
  const adminId = (req as any).user?.sub;  // get authenticated admin ID
  const type = await editType(code, parsed.data, adminId); // pass to service
  res.json(type);
}
...
export async function deleteType(req: Request, res: Response) {
  const { code } = req.params;
  const adminId = (req as any).user?.sub;  // get authenticated admin ID
  await removeType(code, adminId); // pass to service
  res.status(204).send();
}