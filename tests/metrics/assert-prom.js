...
function main() {
  ...
  for (const metric of REQUIRED_METRICS) {
    if (!contents.includes(`"${metric}"`)) {
      console.error(
        `Missing Prometheus metric "${metric}" in metrics middleware.`,
      );
      process.exit(1);
    }
  }

  if (!fileContainsAll(contents, REQUIRED_LABELS.map((l) => `"${l}"`))) {
    console.error(
      "Metrics middleware must expose labels method, route, status_code.",
    );
    process.exit(1);
  }

  if (!fileContainsAll(contents, REQUIRED_BUCKETS.map((b) => `${b}`))) {
    console.error(
      "Histogram buckets must include 0.05, 0.1, 0.2, 0.5, 1, 2, 5 seconds.",
    );
    process.exit(1);
  }
